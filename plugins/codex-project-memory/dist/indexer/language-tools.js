...
export function getLanguageToolDefinition(language) {
...
export function resolveLanguageTool(ctx, language, options = {}) {
...
function lockPath(cachePath) {
    return join(cachePath, "pmem-language-tools.lock.json");
}
function writeToolLock(cachePath, definition) {
    if (!definition.packageName || !definition.version)
        return;
...
function readToolLock(cachePath) {
...
function logToolEvent(ctx, message, details) {
