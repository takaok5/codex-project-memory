...
function regexSymbols(lines, patterns) {
    const symbols = [];
...
function uniqueSymbols(items, fileId) {
    const seen = new Set();
    return items
        .map((item) => {
...
function extractImports(language, lines) {
    const edges = [];
    for (const line of lines) {
        const trimmed = line.trim();
        const sourceModule = importSource(language, trimmed);
        if (!sourceModule || !isLocalDependency(sourceModule))
            continue;
        edges.push({
...
function importSource(language, line) {
...
function extractRoutes(language, lines) {
    const routes = [];
    for (const line of lines) {
        const trimmed = line.trim();
...
//# sourceMappingURL=universal-indexer.js.map