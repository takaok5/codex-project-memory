...
export function createMcpServer(env = { cwd: resolveMcpProjectCwd() }) {
    ...
    register(server, "memory.query", schemas["memory.query"], async (input) => handleMemoryQuery(input, env));
    register(server, "memory.duplicates", schemas["memory.duplicates"], async (input) => handleMemoryDuplicates(input, env));
    register(server, "memory.frame", schemas["memory.frame"], async (input) => handleMemoryFrame(input, env));
    register(server, "memory.refresh", schemas["memory.refresh"], async (input) => handleMemoryRefresh(input, env));
    register(server, "memory.diff", schemas["memory.diff"], async (input) => handleMemoryDiff(input, env));
    register(server, "memory.agent", schemas["memory.agent"], async (input) => handleMemoryAgent(input, env));
    return server;
}
...
function register(server, name, schema, handler) {
