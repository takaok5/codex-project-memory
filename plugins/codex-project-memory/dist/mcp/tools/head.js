...
export async function handleMemoryHead(_input, env) {
    ...
    try {
        ...
        const topModules = db.prepare("SELECT id, name, risk_level FROM modules ORDER BY id ASC LIMIT 8").all().map((row) => ({ id: row.id, name: row.name, riskLevel: row.risk_level }));
        return {
