...
export function buildNormalizedGraph(ctx) {
    ...
    const modules = db.prepare("SELECT * FROM modules ORDER BY id ASC").all().map((row) => ({
        id: row.id,
        name: row.name,
    ...
    const routes = db
        .prepare(`SELECT r.id, r.method, r.path, r.module_id, f.path AS file_path, s.fq_name AS handler
    ...
    const warnings = db
        .prepare(`SELECT w.id, w.warning_type, w.severity, w.module_id, w.message, w.source, w.fingerprint, f.path AS file_path, s.fq_name AS symbol
         FROM warnings w
    ...
    const edges = db
        .prepare(`SELECT e.id, e.edge_kind, e.confidence, fs.path AS source_file, from_s.fq_name AS from_symbol, to_s.fq_name AS to_symbol
         FROM symbol_edges e
    ...
    const duplicateCandidates = db
        .prepare(`SELECT id, kind, similarity, reason
...
function sortBy(rows, keys, descFirst = false) {
