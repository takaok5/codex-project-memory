...
export function getProjectState(db) {
    try {
        const rows = db.prepare("SELECT key, value FROM project_state").all();
...
export function setProjectStateValue(db, key, value) {
    if (!ALLOWED_KEYS.has(key)) {
        throw new PmemError("VALIDATION_ERROR", "Unknown project_state key.");
    }
    try {
        db.prepare("INSERT INTO project_state(key, value, updated_at) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at").run(key, value, nowIso());
    }
    catch (error) {
        throw new PmemError("DB_ERROR", "Project memory database error.", {
            details: { cause: error instanceof Error ? error.message : "unknown" }
...
export function markMemoryDirty(db, reason) {
...
export function markMemoryFresh(db, indexedAt) {
    const tx = db.transaction(() => {
...
export function markMemoryError(db, error) {
    const tx = db.transaction(() => {
...
function parseStatus(value) {
    const allowed = ["not_initialized", "initializing", "fresh", "stale", "dirty", "error"];
    return allowed.includes(value) ? value : "not_initialized";
}
...
//# sourceMappingURL=project-state-repository.js.map