...
export function renderAgentTemplate(name: string): string {
...
export function listAgentTemplates(projectRoot: string): AgentsListOutput {
...
export function installAgentTemplates(projectRoot: string, options: { force?: boolean } = {}): AgentsInstallOutput {
  const dir = join(projectRoot, ".codex", "agents");
  const output: AgentsInstallOutput = { scope: "project", installed: [], skipped: [], overwritten: [] };
  for (const template of AGENT_TEMPLATES) {
    ...
    if (existed) output.overwritten.push(rel);
    else output.installed.push(rel);
  }
  return output;
}
