...
const MANUAL_OVERRIDES: LanguageMetadata[] = [
...
const ID_INDEX = new Map<string, LanguageMetadata>();

  for (const item of LANGUAGES) {
  ID_INDEX.set(item.id, item);
  for (const extension of item.extensions) {
    if (EXCLUDED_EXTENSIONS.has(extension.toLowerCase())) continue;
...
export function detectLanguage(filePath: string): LanguageMetadata | null {
...
function buildLanguageRegistry(): LanguageMetadata[] {
  const byId = new Map<string, LanguageMetadata>();
  for (const item of MANUAL_OVERRIDES) {
    byId.set(item.id, item);
  }
  for (const item of Object.values(linguistLanguages) as LinguistLanguage[]) {
    if (!isSourceLanguage(item)) continue;
    const id = normalizeLanguageId(item.name);
    if (!id || byId.has(id)) continue;
    byId.set(
      id,
      language(id, item.name, [...(item.extensions ?? [])].filter((extension) => !EXCLUDED_EXTENSIONS.has(extension.toLowerCase())), {
...
function isSourceLanguage(language: LinguistLanguage): boolean {
  return language.type === "programming" || language.type === "markup" || language.name === "SQL";
}

function normalizeLanguageId(name: string): string {
...
function language(
  ...
  displayName: string,
  extensions: string[],
  options: { filenames?: string[]; analyzer?: string; detector?: LanguageMetadata["detector"]; topLanguage?: boolean } = {}
): LanguageMetadata {
  return {
    id,
    displayName,
