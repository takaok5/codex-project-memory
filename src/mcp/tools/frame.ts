...
export async function handleMemoryFrame(input: { frame: FrameName }, env: McpToolEnv): Promise<MemoryFrameOutput> {
  const result = await cmdFrame({ cwd: env.cwd, frame: input.frame });
  if (!result.ok || !result.data) throw new PmemError(result.error?.code ?? "FRAME_NOT_FOUND", result.error?.message ?? "Requested memory frame was not found.", { details: result.error?.details });
  return {
