...
export interface ImpactOutput {
  ...
  symbols: Array<{ fqName: string; filePath: string; reason: string }>;
  tests: Array<{ path: string; reason: string }>;
  diagnostics: Array<{ severity: DiagnosticSeverity; filePath: string; message: string; tool: string }>;
...
export interface MemoryCurationOutput {
  mode: "writer_gate";
  accepted: Array<{ kind: EvidenceKind; summary: string; source: string }>;
