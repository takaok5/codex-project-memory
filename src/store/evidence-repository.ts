...
import type {
  ...
  EvidenceFeedbackInput,
  EvidenceFeedbackRecord,
  EvidenceFeedbackSignal,
  EvidenceRecord,
  EvidenceRecordInput,
  EvidenceRecordStatus,
  JsonObject,
  RuntimeEvidenceItemInput,
  RuntimeEvidenceItemRecord,
  RuntimeEvidenceRunInput,
  RuntimeEvidenceRunRecord
} from "../shared/types.js";
import type { MemoryDb } from "./sqlite.js";

export function addRuntimeEvidenceRun(db: MemoryDb, input: RuntimeEvidenceRunInput): { runId: number; itemIds: number[] } {
  const now = nowIso();
  const normalized = {
    kind: input.kind,
  ...
  const result = db
    .prepare(
      `INSERT INTO runtime_evidence_runs(kind, command, status, exit_code, duration_ms, output_summary, created_at)
...
export function listRuntimeEvidenceRuns(db: MemoryDb, limit = 20): RuntimeEvidenceRunRecord[] {
  return (
    db
      .prepare(
        `SELECT id, kind, command, status, exit_code, duration_ms, output_summary, created_at
         FROM runtime_evidence_runs
         ORDER BY created_at DESC, id DESC
         LIMIT ?`
      )
      .all(Math.max(1, limit)) as RuntimeEvidenceRunRow[]
  ).map(runtimeRunFromRow);
}

export function listRuntimeEvidenceItems(db: MemoryDb, filter: { limit?: number; filePath?: string } = {}): RuntimeEvidenceItemRecord[] {
  ...
  return (
    db
      .prepare(
        `SELECT id, run_id, kind, file_path, severity, message, start_line, end_line, fingerprint, created_at
      ...
      .all(params) as RuntimeEvidenceItemRow[]
  ).map(runtimeItemFromRow);
}

export function addEvidenceRecord(db: MemoryDb, input: EvidenceRecordInput): number {
  const now = nowIso();
...
export function addEvidenceRecords(db: MemoryDb, records: EvidenceRecordInput[]): number[] {
  return records.map((record) => addEvidenceRecord(db, record));
}

export function listEvidenceRecords(db: MemoryDb, filter: { status?: EvidenceRecordStatus; kind?: string; limit?: number } = {}): EvidenceRecord[] {
  const clauses: string[] = [];
  const params: Record<string, unknown> = {};
  if (filter.status) {
  ...
  return (
    db
      .prepare(
        `SELECT id, kind, source, summary, file_path, symbol_fq_name, module_id, runtime_run_id, architecture_decision_id,
                confidence, score, status, stale_reason, metadata_json, created_at, updated_at
...
export function markEvidenceStaleForRemovedFiles(db: MemoryDb, filePaths: string[], reason: string): number {
  const normalized = [...new Set(filePaths.map(normalizeOptionalPath).filter((value): value is string => Boolean(value)))];
...
export function upsertArchitectureDecision(db: MemoryDb, input: ArchitectureDecisionInput): number {
  const now = nowIso();
  const params = {
  ...
  db.prepare(
    `INSERT INTO architecture_decisions(title, summary, rationale, status, module_id, file_path, symbol_fq_name, created_at, updated_at)
  ...
  const row = db.prepare("SELECT id FROM architecture_decisions WHERE title = ?").get(params.title) as { id: number };
...
export function listArchitectureDecisions(db: MemoryDb, filter: { status?: EvidenceRecordStatus; limit?: number } = {}): ArchitectureDecisionRecord[] {
  const clauses: string[] = [];
  const params: Record<string, unknown> = {};
  if (filter.status) {
    clauses.push("status = @status");
  ...
  return (
    db
      .prepare(
        `SELECT id, title, summary, rationale, status, module_id, file_path, symbol_fq_name, superseded_by_id, invalidated_by_evidence_id, created_at, updated_at
         FROM architecture_decisions ${where}
         ORDER BY updated_at DESC, id DESC
         LIMIT ${limit}`
      )
      .all(params) as ArchitectureDecisionRow[]
  ).map(decisionFromRow);
}

export function getArchitectureDecision(db: MemoryDb, idOrTitle: number | string): ArchitectureDecisionRecord | null {
  const row =
    typeof idOrTitle === "number"
      ? (db
          ...
          .get(idOrTitle) as ArchitectureDecisionRow | undefined)
      : (db
          .prepare(
            `SELECT id, title, summary, rationale, status, module_id, file_path, symbol_fq_name, superseded_by_id, invalidated_by_evidence_id, created_at, updated_at
             FROM architecture_decisions WHERE title = ?`
          )
          .get(sanitizeText(idOrTitle, 160)) as ArchitectureDecisionRow | undefined);
  return row ? decisionFromRow(row) : null;
}

export function setArchitectureDecisionStatus(db: MemoryDb, id: number, status: EvidenceRecordStatus, reason: string): number {
  const evidenceId = addEvidenceRecord(db, {
    kind: "decision",
    source: "architecture_decision",
  ...
  db.prepare(
    `UPDATE architecture_decisions
     ...
     WHERE id = ?`
  ).run(status, status === "active" ? null : evidenceId, nowIso(), id);
  return evidenceId;
}

export function addEvidenceFeedback(db: MemoryDb, input: EvidenceFeedbackInput): number {
  const params = {
  ...
  const result = db
    .prepare(
      `INSERT INTO evidence_feedback(evidence_id, evidence_key, signal, weight, intent, source, created_at)
...
export function listEvidenceFeedback(db: MemoryDb, limit = 50): EvidenceFeedbackRecord[] {
  return (
    db
      .prepare(
        `SELECT id, evidence_id, evidence_key, signal, weight, intent, source, created_at
         FROM evidence_feedback
         ORDER BY created_at DESC, id DESC
         LIMIT ?`
      )
      .all(Math.max(1, limit)) as EvidenceFeedbackRow[]
  ).map(feedbackFromRow);
}

export function getEvidenceFeedbackScores(db: MemoryDb): Map<string, number> {
  const rows = db.prepare("SELECT evidence_key, SUM(weight) AS score FROM evidence_feedback GROUP BY evidence_key").all() as Array<{ evidence_key: string; score: number }>;
  return new Map(rows.map((row) => [row.evidence_key, Number(row.score)]));
}

export function evidenceRecordFingerprint(input: EvidenceRecordInput): string {
...
function addRuntimeEvidenceItem(db: MemoryDb, runId: number, input: RuntimeEvidenceItemInput, createdAt: string): number {
  const params = normalizeRuntimeItem(runId, input, createdAt);
  const result = db
    .prepare(
      `INSERT INTO runtime_evidence_items(run_id, kind, file_path, severity, message, start_line, end_line, fingerprint, created_at)
...
function normalizeEvidenceRecord(db: MemoryDb, input: EvidenceRecordInput, now: string): EvidenceRecordParams {
  return {
    kind: input.kind,
    source: sanitizeSource(input.source),
    summary: sanitizeText(input.summary, 500),
...
function normalizeRuntimeItem(runId: number, input: RuntimeEvidenceItemInput, createdAt: string): RuntimeItemParams {
...
function existingModuleId(db: MemoryDb, value?: string | null): string | null {
  if (!value) return null;
  const row = db.prepare("SELECT id FROM modules WHERE id = ?").get(value) as { id: string } | undefined;
  return row?.id ?? null;
}
...
function defaultFeedbackWeight(signal: EvidenceFeedbackSignal): number {
  if (signal === "useful" || signal === "accepted" || signal === "opened") return 1;
  if (signal === "not_useful" || signal === "rejected") return -1;
  return 0;
}
...
interface RuntimeEvidenceRunRow {
  id: number;
  kind: RuntimeEvidenceRunRecord["kind"];
  command: string;
  status: RuntimeEvidenceRunRecord["status"];
...
interface RuntimeEvidenceItemRow {
  id: number;
  run_id: number;
  kind: RuntimeEvidenceItemRecord["kind"];
  file_path: string | null;
  severity: RuntimeEvidenceItemRecord["severity"];
  message: string;
...
interface EvidenceRecordParams {
  kind: string;
  source: string;
  summary: string;
...
interface EvidenceRecordRow {
  ...
  summary: string;
  file_path: string | null;
  symbol_fq_name: string | null;
  module_id: string | null;
  runtime_run_id: number | null;
  architecture_decision_id: number | null;
  confidence: number;
  score: number;
  status: EvidenceRecordStatus;
...
interface ArchitectureDecisionRow {
  id: number;
  title: string;
  summary: string;
  rationale: string;
  ...
  invalidated_by_evidence_id: number | null;
  created_at: string;
  updated_at: string;
}

interface EvidenceFeedbackRow {
  id: number;
...
function runtimeRunFromRow(row: RuntimeEvidenceRunRow): RuntimeEvidenceRunRecord {
  return {
    id: row.id,
    kind: row.kind,
    command: row.command,
...
function runtimeItemFromRow(row: RuntimeEvidenceItemRow): RuntimeEvidenceItemRecord {
  return {
    id: row.id,
    runId: row.run_id,
    kind: row.kind,
...
function evidenceFromRow(row: EvidenceRecordRow): EvidenceRecord {
  return {
    id: row.id,
    kind: row.kind,
    source: row.source,
...
function decisionFromRow(row: ArchitectureDecisionRow): ArchitectureDecisionRecord {
  return {
    id: row.id,
    title: row.title,
    summary: row.summary,
...
function feedbackFromRow(row: EvidenceFeedbackRow): EvidenceFeedbackRecord {
  return {
    id: row.id,
...
function safeMetadata(text: string): JsonObject {
  try {
