...
export function upsertLanguageCapability(db: MemoryDb, capability: LanguageCapability): void {
  db.prepare(
  ...
  ).run(toParams(capability));
}

export function listLanguageCapabilities(db: MemoryDb): LanguageCapability[] {
  return (db.prepare("SELECT * FROM language_capabilities ORDER BY language ASC").all() as LanguageCapabilityRow[]).map((row) => ({
...
function toParams(capability: LanguageCapability): Record<string, unknown> {
  return {
...
interface LanguageCapabilityRow {
  language: string;
  display_name: string;
  ...
  diagnostics: number;
  tool: string | null;
  tool_status: LanguageCapability["toolStatus"];
